...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Editar Cliente</h1>
            <p class="text-sm text-slate-500 mt-1">Modificando información de: <span class="font-semibold text-blue-600">{{ $client->company_name }}</span></p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <form action="{{ route('admin.clients.update', $client->id) }}" method="POST" class="p-6 space-y-8">
            @csrf
            @method('PUT')

            <!-- Mantenemos el tipo de facturación oculto para no romper validaciones del controlador -->
            <input type="hidden" name="billing_type" value="{{ $client->billing_type }}">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        <textarea name="address" rows="2" class="block w-full rounded-lg bg-slate-50 border-slate-300 text-slate-800 shadow-sm focus:bg-white focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2.5 px-3 transition-all">{{ old('address', $client->address) }}</textarea>
                    </div>
                </div>
            </div>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center space-x-4">
                            <label class="inline-flex items-center cursor-pointer p-2 rounded-lg hover:bg-white hover:shadow-sm transition-all">
                                <input type="radio" name="is_active" value="1" {{ $client->is_active ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500 h-4 w-4">
                                <span class="ml-2 text-sm font-medium text-slate-700">Activo</span>
                            </label>
                            <label class="inline-flex items-center cursor-pointer p-2 rounded-lg hover:bg-white hover:shadow-sm transition-all">
                                <input type="radio" name="is_active" value="0" {{ !$client->is_active ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500 h-4 w-4">
