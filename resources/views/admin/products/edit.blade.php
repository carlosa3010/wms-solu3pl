...
@section('title', 'Editar SKU')
@section('header_title', 'Gestión de Inventario')

@section('content')
    <!-- Navegación y Título -->
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-slate-800">Actualizar Producto Existente</h2>
        <nav class="flex items-center text-sm text-slate-500 mt-1">
            ...
            <span class="text-slate-400 italic">{{ $product->sku }}</span>
            <i class="fa-solid fa-chevron-right text-[10px] mx-2"></i>
            <span class="font-bold text-slate-700">Editar</span>
        </nav>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="col-span-2">
                            ...
                            <select name="client_id" required class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 ring-custom-primary border-custom-primary outline-none bg-white shadow-sm font-medium transition-all">
                        ...
                        <div>
                            ...
                            <input type="text" name="sku" required value="{{ old('sku', $product->sku) }}" 
                                   class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 ring-custom-primary border-custom-primary outline-none font-mono uppercase transition-all bg-slate-50">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="barcode" value="{{ old('barcode', $product->barcode) }}" 
                                   class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 ring-custom-primary border-custom-primary outline-none font-mono transition-all">
                        </div>
                        ...
                        <div class="col-span-2">
                            ...
                            <input type="text" name="name" required value="{{ old('name', $product->name) }}" 
                                   class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 ring-custom-primary border-custom-primary outline-none transition-all">
                        </div>

                        <div class="col-span-2">
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Descripción Detallada</label>
                            <textarea name="description" rows="3" 
                                      class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 ring-custom-primary border-custom-primary outline-none transition-all">{{ old('description', $product->description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div class="p-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="col-span-2">
                            ...
                            <input type="number" step="0.001" name="weight_kg" value="{{ old('weight_kg', $product->weight_kg) }}" 
                                   class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 ring-custom-primary outline-none font-mono text-right transition-all">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Largo (cm)</label>
                            <input type="number" step="0.1" name="length_cm" value="{{ old('length_cm', $product->length_cm) }}" 
                                   class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 ring-custom-primary outline-none font-mono text-right transition-all">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Ancho (cm)</label>
                            <input type="number" step="0.1" name="width_cm" value="{{ old('width_cm', $product->width_cm) }}" 
                                   class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 ring-custom-primary outline-none font-mono text-right transition-all">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Alto (cm)</label>
                            <input type="number" step="0.1" name="height_cm" value="{{ old('height_cm', $product->height_cm) }}" 
                                   class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 ring-custom-primary outline-none font-mono text-right transition-all">
                        </div>
                        <div>
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div class="p-6">
                        <div id="imagePreview" class="w-full aspect-square bg-slate-100 rounded-lg border-2 border-dashed border-slate-300 mb-4 flex items-center justify-center overflow-hidden">
                            @if($product->image_url)
                                <img src="{{ $product->image_url }}" class="w-full h-full object-cover">
                            @else
                                <i class="fa-solid fa-box text-slate-300 text-4xl"></i>
                            @endif
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                    ...
                    <select name="category_id" class="w-full px-4 py-2 border border-slate-300 rounded-lg outline-none font-medium focus:ring-2 ring-custom-primary border-custom-primary transition-all">
                ...
                <div class="flex flex-col gap-3">
                    <button type="submit" class="w-full bg-custom-primary text-white py-3 rounded-xl font-bold shadow-lg shadow-black/10 hover-bg-primary transition-all flex items-center justify-center gap-2 uppercase tracking-tighter text-sm">
                        <i class="fa-solid fa-save"></i> Guardar Cambios
                    </button>
                    <a href="{{ route('admin.products.index') }}" class="w-full bg-white text-slate-500 py-3 rounded-xl font-bold border border-slate-200 text-center hover:bg-slate-50 transition-all text-sm">
                        Cancelar
                    </a>
                </div>
            </div>
...
@section('scripts')
<script>
    // Vista previa de imagen antes de subir para asegurar que el operario vea el cambio
